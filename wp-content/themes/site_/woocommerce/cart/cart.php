              ...
              <?php //endif ?>
                <?php 
                  ...
                  foreach( $coupon_descs as $amount => $desc ) {
                    ...
                    echo '<span class="minimum-amount '.  ( is_array($current_coupon_archived) && $amount == $current_coupon_archived["minimum_amount"] ? 'current-coupon ' : '' ) . $amount . ( $amount == $minimum_amount ? '' : ' d-none' ) .'"data-desc="'.$desc["description"].'" data-coupon="'.$desc["coupon_code"].'"><b style="text-transform: uppercase;">'.$desc["coupon_code"].'</b> là ' . $desc["description"] . '</span>';
                  }
                ?></p>
