...
$list_brands = site_the_category_brands( $cat->name );
...
// $list_brands = get_field( 'top_brands', 'product-brand_' . $cat->term_id );
    ...
    <div class="brand-highlight row">
      <?php 
        ...
        foreach( $list_brands as $item ):
          ...
          //$url = add_query_arg( 'brands[]', $item->term_id );

          $link = get_term_link( $cat->term_id, $cat->taxonomy ) . '?brands[]=' . $item->term_id;
      ?>
      <div class="col-4 col-lg-2 py-2 brand-item"> 
        <a href="<?php echo esc_url( $link );//$url;?>#top" class="card justify-content-center">
          <div class="p-0 text-center">
            <img src="<?php echo wp_get_attachment_image_url( $image, 'medium' );?>" alt="<?php echo $item->name;?>" class="img-fluid m-h-30-px" />
          </div>
        </a>
      </div>
      <?php endforeach;?>
