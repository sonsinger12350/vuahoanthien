		...
		<?php if ( $show_last_column ) : ?>
			...
			<th class="product-add-to-cart">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo "Ghi chú";//esc_html( apply_filters( 'yith_wcwl_wishlist_view_cart_heading', '', $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
	...
	<?php
		...
		foreach ( $wishlist_items as $item ) :
			...
			if ( $product && $product->exists() ) :
				?>
				<tr class="yith-wcwl-row-item" id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					<?php if ( $show_cb ) : ?>
					...
					<?php if ( $show_quantity ) : ?>
						<td class="product-quantity">
							...
							$disable_class = "";
							if($item->get_product_available() == false ){
								$disable_class = "disabled";
							}
							?>

							<?php if ( ! $no_interactions && $wishlist->current_user_can( 'update_quantity' ) ) : ?>
								<div class="quantity-field d-inline-flex quantity-input-group mx-2">	
									<button type="button" class="btn btn-outline-dark btn-decrease btn-change border fw-bold fs-4 <?php echo $disable_class; ?>">-</button>
									<input <?php echo $disable_class; ?> type="number" min="1" step="1" productid="<?php echo esc_attr( $item->get_product_id() ); ?>" name="items[<?php echo esc_attr( $item->get_product_id() ); ?>][quantity]" value="<?php echo esc_attr( $item->get_quantity() ); ?>" class="quantityInput form-control text-center fw-bold border border-end-0 border-start-0 rounded-0" />
									<button type="button" class="btn btn-outline-dark btn-increase btn-change border fw-bold fs-4 <?php echo $disable_class; ?>">+</button>
								</div>	
							<?php else : ?>
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							//var_dump($item->get_product_available());
							?>
							<?php if ( $item->get_product_available() && $show_add_to_cart && $item->is_purchasable() && 'out-of-stock' !== $item->get_stock_status() ) : ?>
	...
	<?php else :
		...
		<div>
			...
			<div class="wishlist-empty">
				...
				<p><i class="icon-heading bi bi-suit-heart"></i></p>
				<p>Bạn chưa có sản phẩm yêu thích nào !</p>
				<p>Thêm sản phẩm vào Danh sách yêu thích để tìm kiếm và mua sắm dễ dàng hơn.</p>
				<p><a class="btn btn-primary rounded mt-3 mb-3" href="<?php echo get_category_link(100); ?>"><i class="bi bi-bag-heart"></i> Thêm sản phẩm vào Danh sách</a></p>
			</div>
		</div>
		<?php
	endif;
