...
<div class="yith-wcwl-add-button">
	...
	<a href="#add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" rel="nofollow" class="<?php echo esc_attr( $link_classes ); ?> open-pretty-photo" data-rel="prettyPhoto[add_to_wishlist_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>]" data-title="<?php echo esc_attr( apply_filters( 'yith_wcwl_add_to_wishlist_title', $label ) ); ?>">
		...
		if ( apply_filters( 'yith_wcwl_show_text_on_the_image', true ) ) {
			echo '<span class="btn-text">'.wp_kses_post( $label ).'</span>';
		}
		?>
	</a>
	...
	<div id="add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" class="yith-wcwl-popup">
		<form class="yith-wcwl-popup-form a" method="post" action="<?php echo esc_url( wp_nonce_url( add_query_arg( 'add_to_wishlist', $product_id, $base_url ), 'add_to_wishlist' ) ); ?>">
			<div class="yith-wcwl-popup-content">
				...
				<div class="tab choose">
					<p class="popup-description">
						...
						<?php //esc_html_e( 'hay', 'yith-woocommerce-wishlist' ); ?>
						<?php /* ?>
					...
					<div class="yith-wcwl-wishlist-select-container">
						<p class="form-row form-row-wide"> 
							<select name="wishlist_id" class="wishlist-select form-select" id="wishlistselectid">
								<?php if ( empty( $lists ) ) : ?>
									<?php
									/**
				...
				<div class="tab create">
					<p class="popup-description">Tạo danh sách yêu thích mới</p>
					<p class="form-row form-row-wide">
						<?php
						/**
					...
					<p class="d-none form-row form-row-wide">
						<label>
				...
				<!-- w-100 btn btn-outline-primary px-1 border-2 rounded fw-bold fs-10 -->
