...
function site_product_viewed( $post_id = 0 )
{
    ...
    $count = intval( get_post_meta( $post_id, $key ) ) + 1;
    ...
    if( $user_id>0 ) {
        ...
        if( in_array($post_id, $products ) == false ) {
            $products[] = $post_id;
            update_user_meta( $user_id, 'products', implode( ',', $products ) );
        }
    } else {
        site_add_cookie_array( 'products', get_the_ID() );
    }
}
...
{
    return ( $name!='' ? $name .'@' : '' ) . 'viehome.depot';
}
...
function custom_modify_sale_price($price, $product) {
    ...
    if ($product_type === 'gach') {
        ...
        $old_price = $price;
        $new_sale_price = floatval($price) * floatval($heso_quydoi); // Replace this with your actual calculated price
