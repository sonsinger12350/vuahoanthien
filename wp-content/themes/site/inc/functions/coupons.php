...
function site_get_coupons( $params = array() )
{
    ...
    if( empty($coupons[$key]) ) {
        ...
        if( $params['sort'] == 'condition_earn' ) {
            $args['meta_key'] = 'condition_earn';
...
function site_get_checkout_gift($sort = 'ASC', $noType = false)
{
    if( class_exists('WC_Coupon') == false ) return array();
    ...
    if( $list ) {
        if ($noType) {
            foreach( $list as $p ) {
                $data[ $p->post_name ] = new WC_Coupon($p->post_name);
            }
        }
        else {
            foreach( $list as $p ) {
                if (get_post_meta($p->ID, 'discount_type', true) == 'voucher') {
                    $data['vouchers'][ $p->post_name ] = new WC_Coupon($p->post_name);
                }
                else {
                    $data['coupons'][ $p->post_name ] = new WC_Coupon($p->post_name);
                }
            }
        }
    }
...
function site_get_my_gift($user_id, $getAll = false)
{
    $data = [];
    if( class_exists('WC_Coupon') == false ) return $data;
    global $wpdb;

    $sql = "SELECT 
        gift_id, COUNT(gift_id) as total
        FROM {$wpdb->prefix}user_gifts 
        WHERE user_id = %d AND status = 0
    ";
    if (!$getAll) $sql .= " AND type = 'coupon'";
    $sql .= " GROUP BY gift_id";

    $results = $wpdb->get_results($wpdb->prepare($sql, $user_id), ARRAY_A);
    $gifts = array_column($results, 'total', 'gift_id');
    ...
    if (!empty($gifts)) {
        ...
        if (!empty($list)) {
            if ($getAll) {
                foreach( $list as $p ) {
                    $p->img = wp_get_attachment_url($p->_thumbnail_id);
                    $p->total = $gifts[$p->ID];
                    $discount_type = $p->discount_type == 'voucher' ? 'vouchers' : 'coupons';
                    $data[$discount_type][$p->post_name] = $p;
                }
            }
            else {
                foreach( $list as $p ) {
