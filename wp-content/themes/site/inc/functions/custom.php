...
function get_list_product_byIDs_giasohomnay() {
...
function formatNumber($number) {
    if ($number > 1000) {
    	// Calculate the number to one decimal place
	    $formatted_number = round($number / 1000, 1);
	    // Format the number with a single decimal place and append 'N'
	    $result = number_format($formatted_number, 1, ',', '') . 'N';
    } else {
    	$result = $number;
    }
...
function getBrandsByCat($cat, $childCat = null, $price = null) {
	if (empty($cat->taxonomy) || $cat->taxonomy != 'product_cat') return null;
	global $wpdb;

	$listCat = get_term_children($cat->term_id, $cat->taxonomy);
	if (empty($listCat)) $listCat = [$cat->term_id];
	if (!empty($childCat) && !empty(array_intersect($listCat, $childCat))) $listCat = array_intersect($listCat, $childCat);
	...
	if (!empty($price)) {
		...
		if (count($priceArr) > 1) $where = " AND CAST(vhd_postmeta.meta_value AS SIGNED) BETWEEN ".($priceArr[0] * 1000000)." AND ".($priceArr[1] * 1000000);
		else $where = " AND CAST(vhd_postmeta.meta_value AS SIGNED) > ".($priceArr[0] * 1000000);
	...
	$sql = " SELECT GROUP_CONCAT(DISTINCT tr1.term_taxonomy_id)
		...
		WHERE tr1.term_taxonomy_id NOT IN ($listCat) $wherePrice
	";

	$taxonomy = $wpdb->get_var($sql);
	if (empty($taxonomy)) return null;

	$sql = "SELECT vhd_terms.term_id, vhd_term_taxonomy.taxonomy, vhd_terms.name
		...
		AND vhd_terms.term_id IN ($taxonomy)
	";
