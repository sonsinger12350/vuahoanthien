...
if (!empty(WC()->session->get('wps_cart_points'))) $usingPoint = WC()->session->get('wps_cart_points_original');
?>
...
<div class="mb-3 section-bg"> <!-- bg-light border border-0 rounded p-2 -->
	...
	<?php else: ?>
		<div class="point-applying">
			<div id="point-using"><?= !empty($usingPoint) ? "Điểm sử dụng: $usingPoint" : ''?></div>
			<div id="point-remaining"><?= !empty($usingPoint) ? "Điểm còn lại: ".($get_points-$usingPoint) : ''?></div>
		</div>
		<div class="d-flex mb-3">
			<div class="wps_wpr_apply_custom_points input-group me-2">
				...
				<button type="button" class="btn btn-primary rounded" name="wps_cart_points_apply" id="wps_cart_points_apply" value="Apply Points" style="margin-left: 5px;" data-point="<?php echo $get_points; ?>" data-id="1" data-order-limit="0">Áp dụng</button>
			</div>
		</div>
	<?php endif ?>
