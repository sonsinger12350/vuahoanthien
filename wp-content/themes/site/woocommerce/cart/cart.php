...
<form class="cart-form" id="cart-form" action="<?php echo esc_url(wc_get_cart_url()); ?>" method="post"> <!--container-->
	<div class="row">
		<div class="col-12 col-lg-9 col-cart">
									...
									<div class="bg-light row mb-3 py-3 fs-11 bt-1 cart-item <?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)); ?>">
												...
												<div class="product-image">
													<a href="<?php echo $product_permalink; ?>">
														<img src="<?php echo wp_get_attachment_image_url($_product->get_image_id(), 'full'); ?>"
															class="img-fluid rounded-start me-2" alt="" loading="lazy">
													</a>
												</div>
														...
														<div>
															<?php if ($price > 0): ?>
																<span class="cart-item-price text-danger">
																	<?php echo site_wc_price($price); ?>đ
																</span>
																<span class="text-decoration-line-through fs-12"><?php echo site_wc_price($_product->get_regular_price()); ?>đ</span>
															<?php else: ?>
											...
											<div class="inside">
												<?php if ($price > 0): ?>
													<span class="text-decoration-line-through price-normal">
														<?php echo site_wc_price($_product->get_regular_price()); ?><ins class="woocommerce-Price-currencySymbol">đ</ins>
													</span>
													<span class="cart-item-price price-forsale"><?php echo site_wc_price($price); ?><ins class="woocommerce-Price-currencySymbol">đ</ins></span>
												<?php else: ?>
													<span class="cart-item-price">Giá liên hệ</span>
												<?php endif ?>
										...
										<div class="col-2 col-price-total d-md-flex justify-content-center text-danger">
											<?php if ($subtotal > 0): ?>
												...
												<span class="cart_sub_total"><?php echo site_wc_price($subtotal); ?><ins class="woocommerce-Price-currencySymbol">đ</ins></span>
											<?php else: ?>
												<label class="show-on-mobile">Thành tiền: </label>
			...
			<?php wc_get_template_part('cart/cart-gift'); ?>
			<?php if (count($cart_coupons) == 0): ?>
				<div class="section-bg">
					<div class="cart-footer">
						...
						$coupons = site_get_coupons(array('sort' => 'condition_earn'));
						$coupon_descs = [];
						$i = 0;
						...
						foreach ($coupons as $index => $coupon) {
							$condition_earn = get_metadata('post', $coupon->id)['condition_earn'][0];
							$data = $coupon->get_data();
							//var_dump($data);
							$min = (int) $condition_earn; // List all min amount of coupons
							$coupon_descs[$condition_earn] = [
								'description' => $data['description'],
								'coupon_code' => $data['code'],
								'condition_earn' => $condition_earn
							];
							if ($need_amount == 0 && $min > $total) {
								$need_amount = $min - $total;
								$minimum_amount = $condition_earn;
							}

							// $total - Tổng tiền của giỏ hàng 
							// $need_amount - Chi phí tiền cần để đạt mức ưu đãi tiếp theo   
							//var_dump($need_amount);
							//var_dump($minimum_amount);
							if ($minimum_amount <= $total) {
						...
						<div class="row-need-amount<?php echo ($need_amount == 0 && $need_amount != null) ? ' d-none' : ''; ?>"> <!-- bg-light row mb-3 py-3 rounded -->
							<div class="col-md-12 text-center">
								<!-- Mua thêm <span class="need_amount"><?php //echo site_wc_price($need_amount);
																		?></span><sup>đ</sup> để được khuyến mãi! -->
								<p class="cart_coupon_warning" style="display: none;">
								...
								<p class="line-1 <?php echo ($need_amount == 0) ? 'd-none' : ''; ?>" style="display: none">
									...
									// if (is_array($current_coupon_archived) && isset($current_coupon_archived["minimum_amount"])) {
									foreach ($coupon_descs as $amount => $desc) {
										//var_dump($current_coupon_archived);
										echo '<span class="minimum-amount ' . ($amount == $minimum_amount ? 'current-coupon ' : '') . $amount . ($amount == $minimum_amount ? '' : ' d-none') . '" data-desc="' . $desc["description"] . '" data-coupon="' . $desc["coupon_code"] . '"><b style="text-transform: uppercase;">' . $desc["coupon_code"] . '</b> là ' . $desc["description"] . '</span>';
									}
									// } else {
									// Handle the case where $current_coupon_archived is not an array
									// echo "No valid coupon data available.";
									// }  
									?>
								</p>
		...
		<div class="col-12 col-lg-3 cart-infomation">
			<div class="cart-infomation-inside">
				<?php
					...
					if (wc_coupons_enabled()) wc_get_template_part('cart/cart-coupon');
				?>
