...
<?php if (isset($_GET['dev']) && $_GET['dev'] == 1) {
	echo '<div class="dev-info">Product ID: ' . get_the_ID() . '</div>';
} ?>
<div class="bg-light">
	<div class="container">
		<?php
	...
	while (have_posts()) : the_post(); ?>
		...
		<?php
			wc_get_template_part('single-product/bundled-products'); // Same Parent category, Same Brand
			// wc_get_template_part('single-product/combo-products');
		?>
		<div class="section">
			<div class="container">
				<?php wc_get_template_part('single-product/info'); ?>
			</div>
		</div>
		<?php get_template_part('woocommerce/section/related-products', '', array('living' => 'space')); ?>
		<?php wc_get_template_part('section/related-products'); // Same Category ?>
		<?php wc_get_template_part('section/product-brand');  // Same Brand ?>

		<div class="container">
			<?php wc_get_template_part('section/product-viewed'); ?>
		</div>

		<?php //wc_get_template_part( 'section/related-search' ); ?>

		<?php wc_get_template_part('single-product/review'); ?>
