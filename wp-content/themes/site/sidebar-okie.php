...
// Function to fetch all level 1 categories with published products, excluding specific categories
function fetch_all_level_1_categories_with_products() {
    $exclude_slugs = ['gia-soc-hom-nay', 'chua-phan-loai', 'decor']; // Add the slugs of categories you want to exclude
                    ...
                    <li class="nav-item my-2">
                        ...
                        <?php if ($brands): ?>
                            <div class="explore-more <?php echo count($brands) > 7 ? '' : 'tooShort'; ?>">
                                <ul class="nav flex-column <?php echo count($brands) > 7 ? 'collapse' : ''; ?>" id="detailMoreBrands">
                                ...
                                <ul class="nav flex-column collapse" id="detailMoreCate">
                                    <?php if (count($terms) > 0): ?>
                                        <?php foreach ($terms as $term): ?>
                                            <li>
                                                <?php $category_link = get_term_link($term->term_id); ?>
                                                <?php if (($level == 1 || $level == 3 || ($value_brands && $level == 0)) && !is_product_category('gia-soc-hom-nay')): ?>
